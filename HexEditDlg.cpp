	...
	: CXTPResizeDialog(CHexEditDlg::IDD, pParent)
{
	...
	m_pData = (LPBYTE) // don't use _T()
        _T("This is some test data for CXTHexEdit without any special meaning. ")
        _T("You can type 10 characters past the end.");
...
BOOL CHexEditDlg::OnInitDialog()
{
    ...
    const int len = _tcsclen((_TCHAR *)m_pData);
	...
	//m_txtEnabled.SetFont(&xtAfxData.fontBold);
	//m_txtDisabled.SetFont(&xtAfxData.fontBold);
...
void CHexEditDlg::OnChangeBaseAddress() 
{
	...
	_stscanf_s(LPTSTR(LPCTSTR(cs)), _T("%lx"), &dwAddress);
...
void CHexEditDlg::SetFileData(CString fileNamePathStr)
{
	CString contentStr = NLPReadFile(fileNamePathStr);
	int len = contentStr.GetLength();
	m_buff = (LPBYTE)malloc((len+1)*2);
	memcpy(m_buff,(const _TCHAR *)contentStr,len);
	m_editHex.SetData(m_buff,len);
	SetOptions(true,true,true,true);
}

void CHexEditDlg::SetData(LPBYTE buf, int len)
{
	m_editHex.SetData(buf,len);
}

void CHexEditDlg::SetOptions(BOOL a, BOOL h, BOOL c, BOOL w)
{
	m_buff = NULL;
	m_bAddress = a;
	m_bHex = h;
