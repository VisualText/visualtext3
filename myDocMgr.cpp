...
void CMyDocManager::UnregisterShellFileTypes()
{
...
void CMyDocManager::RegisterShellFileTypes(BOOL bCompat)
{
	ASSERT(!m_templateList.IsEmpty());  // must have some doc templates
...
void CMyDocManager::AddDocTemplate(CDocTemplate* pTemplate)
{
	if (pTemplate == NULL)
	{
		//if (pStaticList != NULL)
...
POSITION CMyDocManager::GetFirstDocTemplatePosition() const
{
	return m_templateList.GetHeadPosition();
}

CDocTemplate* CMyDocManager::GetNextDocTemplate(POSITION& pos) const
{
	return (CDocTemplate*)m_templateList.GetNext(pos);
}

BOOL CMyDocManager::SaveAllModified()
{
	POSITION pos = m_templateList.GetHeadPosition();
	while (pos != NULL)
	{
...
//			return FALSE;
		...
		// SRP 010517
		// The following is slightly modified code for MFC virtual CDocTemplate::SaveAllModified()
		// VT prompts to save a modified document twice when the entire app is shut down,
		// if a document happens to still be open.
		// First prompt is here.
		// Second prompt is in CVisualTextDoc::OnCloseDocument()
		...
		while (pos != NULL)
		{
			...
			if (!pDoc->SaveModified())
				return FALSE;
			// if still modified and a VisualTextDoc, set the prompt flag to false
			if (pDoc->IsModified() && pDoc->IsKindOf(RUNTIME_CLASS(CVisualTextDoc)) )
				((CVisualTextDoc*)pDoc)->m_bPromptSaveModified = false;
		}
	}
	return TRUE;
}

void CMyDocManager::CloseAllDocuments(BOOL bEndSession)
{
	POSITION pos = m_templateList.GetHeadPosition();
	while (pos != NULL)
	{
...
int CMyDocManager::GetDocumentCount()
{
	// count all documents
	int nCount = 0;
	POSITION pos = m_templateList.GetHeadPosition();
...
BOOL CMyDocManager::OnDDECommand(LPTSTR lpszCommand)
{
	...
	if (ThisApp->m_pCmdInfo != NULL)
	{
		ThisApp->m_nCmdShow = (int)AfxGetApp()->m_pCmdInfo;
		ThisApp->m_pCmdInfo = &cmdInfo;
	}
	else
		pOldInfo = ThisApp->m_pCmdInfo;

	if (cmdInfo.m_nShellCommand == CCommandLineInfo::FileOpen)
	{
		// show the application window
		CVisualTextApp *ThisApp =(CVisualTextApp *)AfxGetApp() ;
		CWnd* pMainWnd = ThisApp->m_pMainWnd;
		int nCmdShow = ThisApp->m_nCmdShow;
		...
		ThisApp->CVisualTextApp::OpenDocumentFile(cmdInfo.m_strFileName);
	...
	pDoc = ThisApp->CVisualTextApp::OpenDocumentFile(cmdInfo.m_strFileName);
	ThisApp->m_pCmdInfo = &cmdInfo;
	ThisApp->m_pMainWnd->SendMessage(WM_COMMAND, ID_FILE_PRINT_DIRECT);
	ThisApp->m_pCmdInfo = NULL;
	...
	if (GetDocumentCount() > nOldCount)
	 ...
	 if (!AfxOleGetUserCtrl())
		ThisApp->m_pMainWnd->PostMessage(WM_CLOSE);

RestoreAndReturn:
	ThisApp->m_pCmdInfo = pOldInfo;
	return bRetVal;
}


void CMyDocManager::OnFileNewTemplateDoc(CDocTemplate* pTemplate)
{
	if (!pTemplate) 
		return;
...
void CMyDocManager::OnFileOpen()
{
	...
	CString newName;
	CVisualTextApp *ThisApp = (CVisualTextApp *)AfxGetApp() ;
	if (!ThisApp->DoPromptFileName(newName, AFX_IDS_OPENFILE,
	  OFN_HIDEREADONLY | OFN_FILEMUSTEXIST, TRUE, NULL))
		return; // open cancelled
	ThisApp->CVisualTextApp::OpenDocumentFile(newName);
		// if returns NULL, the user has already been alerted
}
	...
	*/
	...
	if (pOpenDocument != NULL)
	{
		...
		if (pos != NULL)
		{
			...
			CFrameWnd* pAppFrame;
			CVisualTextApp *ThisApp =(CVisualTextApp *)AfxGetApp() ;
			if (pFrame != (pAppFrame = (CFrameWnd*)ThisApp->m_pMainWnd))
			{
...
int CMyDocManager::GetOpenDocumentCount()
{
	int nOpen = 0;
	POSITION pos = m_templateList.GetHeadPosition();
	while (pos != NULL)
