...
Copyright © 2001 by Text Analysis International, Inc.
All rights reserved.  No part of this document may be copied, used, or
modified without written permission from Text Analysis International,
Inc.
*******************************************************************************/
...
extern CString appAnaPathStr;
extern CString appStrName;
...
IMPLEMENT_DYNCREATE(CTreeViewAnalyzer, CXTTreeCtrl)
...
CTreeViewAnalyzer& CTreeViewAnalyzer::Get()
{
	static CTreeViewAnalyzer theTree;
	return theTree;
}

BOOL CTreeViewAnalyzer::Create(CWnd* pParentWnd)
{
	if (::IsWindow(m_hWnd))
	{
...
BEGIN_MESSAGE_MAP(CTreeViewAnalyzer, CXTTreeCtrl)
...
int CTreeViewAnalyzer::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CTreeViewAnalyzer::LoadKBAnalyzer(CONCEPT *concept, HTREEITEM parentItem)
{
	...
	while (concept) {
		...
		kbPassNum = KBGetValueLong(concept,_T("pass"));
		if (kbPassNum < 0)
		...
		newItem = InsertItem(nameStr,image,imageSelected,parentItem,TVI_LAST);
		SetItemData(newItem,(DWORD)concept);
...
void CTreeViewAnalyzer::FillListWithSequence()
{
	...
	while (pass) {
		...
		if (algoStr == _T("pat") && !FileExists(rulesPathStr))
			mainFrm->AddDebugLine(_T("ERROR>>> no analyzer file"),passStr);
...
//		kbDirty = true;
		...
		item = InsertItem(nameStr,image,imageSelected,parentItem,TVI_LAST);
		SetItemData(item,(DWORD)passConcept);
...
void CTreeViewAnalyzer::InitAnalyzer()
{
	...
	if (!nlp) {
		...
		_TCHAR appDir[1001];
		_tcscpy(appDir,(const _TCHAR *)AppPath());
		_TCHAR analDir[1001];
		_tcscpy(analDir,(const _TCHAR *)AnalyzerSequencePath());
...
void CTreeViewAnalyzer::EditRulesFile(LPCTSTR lpRulesPath)
{
...
//			else if (AfxMessageBox(msgStr, MB_YESNO) == IDYES) { // SRP 010321 Don't ask the question, per AM. Superfluous.
		...
		if (doc) {
			if (lpRulesPath) {
				...
				doc->m_concept = NULL;
				if (hItem) {
					doc->m_concept = (CONCEPT *)GetItemData(hItem);
...
void CTreeViewAnalyzer::RenumberPasses(CONCEPT *concept)
{
	...
	RenumberPasses(CalculatePassNumber(concept),PassCount());
}
	...
	while (c) {
		...
		if (type != ANAL_PASS_FOLDER) {
			if (passNumber >= fromPass) {
				...
				SetItemText(item,DisplayPassName(c));
			}
			passNumber++;
		}
		c = cg->Next(c);
...
int CTreeViewAnalyzer::RecalculateFolders(HTREEITEM item)
{
	...
	while (item) {
		concept = (CONCEPT *)GetItemData(item);
		...
		if (grandchildCount >= 0) {
			...
			StringToChar(commentBuff,commentStr,commentStr.GetLength());
			Aseq::setComment(nlp,CalculatePassNumber(concept,false),commentBuff);
...
bool CTreeViewAnalyzer::InStub(HTREEITEM itemIn) 
{
	...
	CString itemStr = GetItemText(itemIn);
...
int CTreeViewAnalyzer::GetPassType(int pass) 
{
	...
	CONCEPT* concept = (CONCEPT *)GetItemData(item);
	if (!concept)
...
void CTreeViewAnalyzer::GetPassInfo(int passNum) 
{
	...
	m_itemSelected = GetSelectedItem();
	m_itemParent = GetParentItem(m_itemSelected);
	m_boolInStub = InStub();
	m_conceptSelected = (CONCEPT *)GetItemData(m_itemSelected);
	CString debugStr = ConceptName(m_conceptSelected);
	m_intPass = CalculatePassNumber(m_conceptSelected,false);
	m_intAseqPass = CalculatePassNumber(m_conceptSelected,false);
...
void CTreeViewAnalyzer::AddExistingPass(CString filePathStr) 
{
	...
	if (wnd->AnalyzerCreated()) {
		...
		for (int n = max - 1; n >= 0; --n) { // To preserve picked order...
			...
			if (FileHasExtension(filePathStr,_T("pat"))) {
									   ...
									   bool active, bool treeInsert, bool editLabelFlag, bool renumberFlag)
{
	...
	int passNum = CalculatePassNumber(c) + 1;

	// ADJUST CONCEPT FOR FOLDER AND KB WHICH IS FLAT
	int aseqPass = m_intAseqPass;
	if (m_passType == ANAL_PASS_FOLDER && !folderEmpty) {
		c = lastPassConcept;
		aseqPass = CalculatePassNumber(c,false);
	}
	...
	_tcscpy(algoBuff,(const _TCHAR *)algoStr);
	_tcscpy(dataBuff,(const _TCHAR *)dataStr);
	_tcscpy(commentBuff,(const _TCHAR *)commentStr);
	...
	if (treeInsert) {
		...
		if (m_passType == ANAL_PASS_FOLDER) {
			if (m_boolTopLevel) {
				...
				HTREEITEM firstItem =
					TreeCtrlFirstSibling(this,GetChildItem(m_itemSelected));
		...
		newItem = InsertItem(textStr,image,imageSelected,m_itemParent,afterItem);
		SetItemData(newItem,(DWORD)itemConcept);
		SelectItem(newItem);
...
void CTreeViewAnalyzer::DeleteGrammarStub()
{
	...
	CTreeViewGrammar::Get().DeleteStubByName(itemStr);
}

void CTreeViewAnalyzer::DeletePass()
{
	// TO BE TEMPORARY STUFF
	APASS *pass = Aseq::excisePass(nlp,m_intAseqPass);
	Aseq::deletePass(pass);

	// REMOVE FROM KB
	CONCEPT *nextConcept = cg->Next(m_conceptSelected);
	cg->rmConcept(m_conceptSpec,ConceptNumber(m_conceptSelected));
	
	// REMOVE FROM ANALYZER TAB
	DeleteItem(m_itemSelected);
	if (GetCount() == 0)
		ResetVars();
	else
		RenumberPasses(nextConcept);
...
void CTreeViewAnalyzer::DeleteStub()
{
	...
	while (item) {
		prevItem = GetPrevSiblingItem(item);
		SelectItem(item);
		DeletePass();
		if (item == startItem)
			break;
...
int CTreeViewAnalyzer::GetPassCount()
{
	CString itemStr;
	HTREEITEM item = GetRootItem();
	for (int i=0; item && i <= 10000; i++)
...
{
	CString cleanNameStr = GetItemText(item);
...
{
	CString numStr = GetItemText(item);
	int pos = numStr.Find(' ');
	if (pos >= 0) {
...
void CTreeViewAnalyzer::OnBegindrag(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = 0;
	...
	m_itemParent = GetParentItem(m_hitemDrag);
	...
	if (m_bLDragging) {
		...
		if (hItem) {
			...
			if (IsDropSource(m_hitemDrop)) {
				...
				else if (m_destinationType == MOVE_BELOW && GetPrevItem(m_hitemDrop) != GetPrevSiblingItem(m_hitemDrop))
					topSensitive = true;
				else if (m_destinationType == MOVE_ABOVE && GetNextItem(m_hitemDrop) != GetNextSiblingItem(m_hitemDrop))
					bottomSensitive = true;
				...
				if (m_hitemDrop != lastDropItem || topSensitive || bottomSensitive) {
					...
					if (topSensitive && abs(point.y - rect.top) <= 3)
						...
						if (GetItemState(m_hitemDrop,TVIS_EXPANDED) & TVIS_EXPANDED && GetChildItem(m_hitemDrop))
							m_destinationType = MOVE_BELOW;
						else
...
void CTreeViewAnalyzer::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (m_bLDragging) {
		...
		MoveTreeItem(m_hitemDrag,m_hitemDrop,GetParentItem(m_hitemDrop));
	}
	cg->setDirty(save);	// Kb status unaffected.	// 07/11/03 AM.
}
...
void CTreeViewAnalyzer::RunAnalyzer(CString filePathStr, int level)
{
	...
	theApp.BeginWaitCursor();	// Trying this here.	// 06/24/02 AM.
	...
	if (m_dirtyFlag) {
		...
		if (m_dirtyFlag) {	// Didn't intern correctly.	// 05/19/01 AM.
			wnd->AddDebugLine(_T(">>>SYNTAX ERRORS"),_T("NOT ANALYZING TEXT")); // 05/19/01AM.
			// TODO: REMOVE FILES FROM _LOG FOLDER.	// 05/19/01 AM. 
			theApp.EndWaitCursor();	// 06/24/02 AM.
...
//		theApp.BeginWaitCursor();		// Starts too late.	// 06/24/02 AM.
...
LRESULT CTreeViewAnalyzer::OnAnalyzeOutputMessage(WPARAM wParam, LPARAM lParam)
{
	...
	if (index < m_AnalyzeOutputStrDirs[0].GetSize() && !m_AnalyzeOutputStrDirs[0][index].IsEmpty())
		wnd->AddDebugLine(m_AnalyzeOutputStrDirs[0][index], m_AnalyzeOutputStrDirs[1][index], true);
	if (index < m_AnalyzeOutputStrFiles[0].GetSize() && !m_AnalyzeOutputStrFiles[0][index].IsEmpty())
		wnd->AddDebugLine(m_AnalyzeOutputStrFiles[0][index], m_AnalyzeOutputStrFiles[1][index], true);
	return 0L;
}
...
LRESULT CTreeViewAnalyzer::OnAnalyzeOutputPassMessage(WPARAM wParam, LPARAM lParam)
{
	...
	if (passStr != _T("") && pass)
		wnd->AddDebugLine(msgStr, passStr, true);
	return 0L;
}
...
//	if (m_OkAnalysis)	// 06/19/03 AM.
		...
		CTreeViewText::Get().SetTextAsAnalyzed(m_AnalyzeFiles[index]);
	if (appBoolVerbose && index < m_AnalyzeOutputStrFiles[0].GetSize() && !m_AnalyzeOutputStrFiles[0][index].IsEmpty()) {
		...
		wnd->AddDebugLine(m_AnalyzeOutputStrFilesTime[index], m_AnalyzeOutputStrFiles[1][index], true);
	}
	m_AnalyzePass = 0;
...
//	logmsg("OnAnalyzeCleanup:");
	...
	if (bCompleted) {
		wnd->AddDebugLine(m_AnalyzeOutputFinalMsgStr, m_AnalyzePathStr);
		wnd->AddErrorToDebug();
	}
...
void CTreeViewAnalyzer::ProcessLog(CString fileName)
{
	CMainFrame *wnd = (CMainFrame *)AfxGetMainWnd();
	if (!appBoolBatchMode)
		CTreeViewText::Get().MoveLogFiles(fileName);
...
void CTreeViewAnalyzer::SetPassDirty(CString rulePathStr)
{
	...
	else if (FileHasExtension(rulePathStr,_T("pat"))) {
...
{
	return StateString(this,GetRootItem(),STATE_GET_SELECTED);
}

void CTreeViewAnalyzer::SetSelection(CString selStr)
{
	SetRedraw(false);
	StateFromString(this,GetRootItem(),STATE_SELECT,selStr);
	SetRedraw(true);
}
...
CString CTreeViewAnalyzer::TreePath(HTREEITEM child)
{
	...
	while (parent = GetParentItem(child)) {
...
void CTreeViewAnalyzer::OnEndlabeledit(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (m_itemSelected) {
		CONCEPT *concept = (CONCEPT *)GetItemData(m_itemSelected);
...
bool CTreeViewAnalyzer::RenamePass(CONCEPT *concept, CString newNameStr) 
{
	...
	if (IsConceptFirstChild(concept,m_conceptSpec)) {
		_tcscpy(data,Aseq::getData(nlp,m_intAseqPass));
	}
	...
	CString debugStr = GetItemText(m_itemSelected);
	CString displayNameStr = DisplayPassName(m_conceptSelected);
	SetItemText(m_itemSelected,displayNameStr);
	...
	Aseq::setData(nlp,m_intAseqPass,data);
...
void CTreeViewAnalyzer::SetPassActive(CONCEPT *concept, bool activeFlag) 
{
	...
	int pass = KBGetValueLong(concept,_T("pass"));
	...
	Aseq::setActive(nlp,CalculatePassNumber(concept),active);
	...
	if (activeFlag) {
		...
		SetItemImage(conceptTreeItem,image,imageSelected);
	}
	else
		SetItemImage(conceptTreeItem,14,14);
}
...
void CTreeViewAnalyzer::RetypePass(CONCEPT *concept, ANAL_PASS_TYPE type) 
{
	...
	if (type != ANAL_PASS_FOLDER && IsConceptFirstChild(concept,m_conceptSpec)) {
		_TCHAR algo[1001];
		StringToChar(algo,algoStr,1000);
		int pass = CalculatePassNumber(concept);
		Aseq::setAlgo(nlp,pass,algo);
		_tcscpy(algo,Aseq::getAlgo(nlp,pass));
	}
...
HTREEITEM CTreeViewAnalyzer::TreeItemFromConcept(CONCEPT *concept, HTREEITEM item)
{
	...
	while (item) {
		c = (CONCEPT *)GetItemData(item);
		debugStr = GetItemText(item);
		if (c == concept)
			return item;
		if ((child = GetChildItem(item)) &&
...
void CTreeViewAnalyzer::MoveSelUp()
{
	if (CanMoveSelectedUp()) {
		m_destinationType = MOVE_ABOVE;
		MoveTreeItem(m_itemSelected,GetPrevSiblingItem(m_itemSelected));
	}
//		MovePass(false);
}
...
void CTreeViewAnalyzer::MoveSelDown()
{
	if (CanMoveSelectedDown()) {
		m_destinationType = MOVE_BELOW;
		HTREEITEM toItem = GetNextSiblingItem(m_itemSelected);
		if (TypeFromItem(toItem) == ANAL_PASS_FOLDER)
			m_destinationType = MOVE_PAST_FOLDER;
		MoveTreeItem(m_itemSelected,toItem,GetParentItem(toItem));
	}
//		MovePass(true);
}
...
void CTreeViewAnalyzer::MovePass(bool down)
{
	...
	if (down) {
		if (m_boolInStub) {
			...
			HTREEITEM nextItem = GetNextSiblingItem(m_itemSelected);
			...
			if (InStub(nextItem)) {
				...
				startConcept = cg->Prev(startConcept);
				RenumberPasses(startConcept);
				nextItem = GetNextSiblingItem(endItem);
				SelectPass(TreeItemNumberFromItem(this,nextItem)-1);
			}
			else {
				cg->moveCright(m_conceptSelected);
				Aseq::moveDown(nlp,m_intAseqPass);
				MoveTreeNodeDown(this,m_itemSelected);
		...
		if (m_boolInStub) {
			...
			GetStubBoundingItems(m_itemSelected,startItem,endItem);
			beforeStartItem = GetPrevSiblingItem(startItem);
			...
			HTREEITEM prevItem = GetPrevSiblingItem(m_itemSelected);

			if (InStub(prevItem)) {
				GetStubBoundingItems(prevItem,startItem,endItem);
				MoveTreeNodesDown(startItem,endItem);
				RenumberPasses();
				prevItem = GetPrevSiblingItem(startItem);
				SelectPass(TreeItemNumberFromItem(this,prevItem)-1);
			}
			else {
				cg->moveCleft(m_conceptSelected);
				Aseq::moveUp(nlp,m_intAseqPass);
				MoveTreeNodeUp(this,m_itemSelected);
				RenumberPasses();
			}
		}
	}
...
HTREEITEM CTreeViewAnalyzer::MoveTreeItem(HTREEITEM fromItem, HTREEITEM toItem, HTREEITEM toParent)
{
	...
	if (m_destinationType == MOVE_PAST_FOLDER) {
		toItem = GetNextSiblingItem(toItem);
		if (toItem) {
	...
	if (m_destinationType == MOVE_ABOVE) {
		HTREEITEM prevItem = GetPrevSiblingItem(toItem);
		m_destinationType = MOVE_BELOW;
		if (prevItem) {
			toConceptItem = GetPrevItem(toItem);  // THE CONCEPT NEEDS THE IMMEDIATELY PREVIOUS TREE ITEM
			toItem = prevItem;
		...
		// INSERTING AFTER A FOLDER
			...
			if ((int)KBGetValueLong(concept,_T("itemcount")) > 0) {
				...
				toConceptItem = GetParentItem(toItem);
			}
		}
	}
	else if (m_destinationType == MOVE_BELOW && toItemCount > 0 && toType == ANAL_PASS_FOLDER) {
		toItem = GetChildItem(toItem);
		toConcept = GetItemInfo(toItem,toPass,toType,toItemCount);
		insertFirstFlag = true;
	}

	// IF PASS NUMBERS ARE -1 BECAUSE OF FOLDERS, THEN FIND NEAREST PASS NUMBER
	if (fromType == ANAL_PASS_FOLDER && fromPass == -1) {
		if (cg->Next(fromConcept))
			fromPass = (int)KBGetValueLong(cg->Next(fromConcept),_T("pass"));
		else
			fromPass = (int)KBGetValueLong(cg->Prev(fromConcept),_T("pass"));
	}
	if (toType == ANAL_PASS_FOLDER && toPass == -1)
		toPass = (int)KBGetValueLong(cg->Next(toConcept),_T("pass"));
	...
	if (m_destinationType == MOVE_INTO || m_destinationType == MOVE_PAST_FOLDER) {
		for (int i=0; i<toItemCount; i++) {
			toPass++;
			toConceptItem = GetNextItem(toConceptItem);
		}
	}
	...
	UINT isExpanded = GetItemState(fromItem,TVIS_EXPANDED) & TVIS_EXPANDED;

	if (insertFirstFlag)
		toItem = TVI_FIRST;
	if (m_destinationType == MOVE_PAST_FOLDER && toType == ANAL_PASS_FOLDER)
		fromCopy = CopyBranch(this,fromItem,toParent,TVI_LAST);
	else if (m_destinationType == MOVE_INTO && toType == ANAL_PASS_FOLDER)
		fromCopy = CopyBranch(this,fromItem,toItem,TVI_LAST);
	else
		fromCopy = CopyBranch(this,fromItem,toParent,toItem);
	DeleteItem(fromItem);
...
{
	CONCEPT *concept = (CONCEPT *)GetItemData(item);
	pass = (int)KBGetValueLong(concept,_T("pass"));
...
{
	CONCEPT *fromConcept = (CONCEPT *)GetItemData(fromItem);
	CONCEPT *toConcept = (CONCEPT *)GetItemData(toItem ? toItem : toParent);

	int fromPass = CalculatePassNumber(fromConcept,false);
...
{
	HTREEITEM beforeStartItem = GetPrevSiblingItem(startItem);
	HTREEITEM afterEndItem = GetNextSiblingItem(endItem);
...
{
	return StateString(this,GetRootItem(),STATE_GET_EXPANDED);
}

void CTreeViewAnalyzer::SetState(CString stateStr)
{
	SetRedraw(false);
	StateFromString(this,GetRootItem(),STATE_EXPAND_COLLAPSE,stateStr);
	SetRedraw(true);
}
...
void CTreeViewAnalyzer::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (m_passType != ANAL_PASS_PAT || !CTreeViewText::Get().OpenProcessedFiles(_T("log"),false)) {
...
void CTreeViewAnalyzer::OnSequencemenuAddStub() 
{
	...
	if (wnd->AnalyzerCreated() && AskForText(_T("Enter stub name:")))
		CTreeViewAnalyzer::Get().AddStub(appAskText);
}
...
void CTreeViewAnalyzer::AddStub(CString stubNameStr, bool addAfterFlag) 
{
	if (stubNameStr != _T("")) {
		...
		if (StubAlreadyExists(stubNameStr)) {
			...
			CMainFrame *wnd = (CMainFrame *)AfxGetMainWnd();
			if (wnd->GetActiveTab() == WORKSPACE_ANALYZER)
				CTreeViewGrammar::Get().AddTopLevelConcept(stubNameStr,false);
			m_dirtyFileFlag = true;
			//SetRedraw(true);
		}
	}
...
bool CTreeViewAnalyzer::StubAlreadyExists(CString conceptStr) 
{
	...
	while (item) {
		...
		GetItemImage(item,image,selImage);
...
void CTreeViewAnalyzer::HighlightRuleMatches(CStringList &selList, CString logPathStr, CString conceptStr) 
{
	...
	FILE *fp;
//	if (!(fp = fopen((const char *)logPathStr,"r"))) {	// 10/03/03 AM.
	if (!(fp = _tfopen((const _TCHAR *)logPathStr,_T("rb")))) {	// Binary mode. // 10/03/03 AM.
#ifdef _DEBUG
		afxDump << _T("File could not be opened ") << logPathStr << _T("\n");
#endif
		return;
	}
	...
	CMainFrame *wnd = (CMainFrame *)AfxGetMainWnd();
	CString textStr = NLPReadFile(InputPath(wnd->GetProcessText()));
	CString lineStr;
	CString conStr;
	CString rangeStr;
	CString semStr;
	...
	while (_fgetts(line,10000,fp)) {
		...
		ParseLogLine(ruleTextStr,lineStr,lineCount,startSel,endSel,passNum,lineNum,
			indent,conStr,semStr,rangeStr,firedFlag,builtFlag);
...
int CTreeViewAnalyzer::SelectPassFromConcept(CONCEPT *concept) 
{
	...
	if (item) {
		SelectItem(TreeItemFromConcept(concept));
...
bool CTreeViewAnalyzer::CanMoveSelectedUp() 
{
	...
	if (m_boolInStub) {
		...
		GetStubBoundingItems(m_itemSelected,startItem,endItem);
		firstItem = TreeCtrlFirstSibling(this,startItem);
		if (firstItem == startItem)
			return false;
	}

	// DEFAULT CHECK
	HTREEITEM firstSibling = TreeCtrlFirstSibling(this,m_itemSelected);
...
bool CTreeViewAnalyzer::CanMoveSelectedDown() 
{
	...
	if (m_boolInStub) {
		...
		GetStubBoundingItems(m_itemSelected,startItem,endItem);
		lastItem = TreeCtrlLastSibling(this,endItem);
		if (lastItem == endItem)
			return false;
	}

	bool canMove = GetNextSiblingItem(m_itemSelected) ? true : false;
	return canMove;
}
...
//		kbDirty = true;
		...
		HTREEITEM item;
		bool wasEmpty = GetChildItem(itemParent) ? false : true;
		item = InsertItem(MAINSTR,12,13,itemParent,TVI_FIRST);
		SetItemData(item,(DWORD)mainConcept);
		if (wasEmpty)
			Expand(m_itemParent,TVE_EXPAND);
...
CString CTreeViewAnalyzer::PassFileFromNumber(int passNum)
{
	...
	while (c) {
		pass = (int)KBGetValueLong(c,_T("pass"));
		if (pass == passNum)
			break;
		c = cg->Next(c);
	}
...
int CTreeViewAnalyzer::CalculatePassNumber(CONCEPT *concept, bool skipFoldersFlag)
{
	int passCount = 0;
	CONCEPT *c = cg->Down(m_conceptSpec);
	ANAL_PASS_TYPE type = ANAL_PASS_NONE;
	...
	while (c) {
		type = (ANAL_PASS_TYPE)KBGetValueLong(c,_T("type"));
		if ((!skipFoldersFlag && type == ANAL_PASS_FOLDER) ||
			type == ANAL_PASS_PAT || type == ANAL_PASS_REC || type == ANAL_PASS_SYSTEM)
			passCount++;
		if (c == concept)
			break;
		c = cg->Next(c);
	}
	if (type == ANAL_PASS_FOLDER && skipFoldersFlag)
		passCount++;
	return passCount;
}

int CTreeViewAnalyzer::PassCount()
{
	int passCount = 0;
	CONCEPT *c = cg->Down(m_conceptSpec);
	ANAL_PASS_TYPE type = ANAL_PASS_NONE;
...
int CTreeViewAnalyzer::NumberOfItems()
{
	return GetCount();
}
...
//	char buf[1001];	// 10/29/02 AM.
	CMainFrame *wnd = (CMainFrame *)AfxGetMainWnd();
	CTreeViewAnalyzer* pView = (CTreeViewAnalyzer*)pParam;
	void* parse = 0;
...
//			sprintf(buf,"End suspend1. maxdword=%ld,suspendcount=%ld",
...
//            logmsg("Ended thread.");	// 10/29/02 AM.
		...
		CString textStr;
		bool response = false;
		_tcscpy(appDir,AppPath());
		_tcscpy(infile, pView->m_AnalyzeFiles[pView->m_AnalyzeFileIndex]);
		_tcscpy(outfile, OutputPath(_T("output.txt")));
...
//					sprintf(buf,"Completing thread. maxdword=%ld,suspendcount=%ld",
//							MAXDWORD,suspendCount);
